<?php
    /**
     * @package POWr Banner Slider
     * @version 1.6.4
     */
    /*
    Plugin Name: POWr Banner Slider
    Plugin URI: https://www.powr.io/plugins/banner-slider
    Description: Beautiful sliding banners for images and video.  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////Create Banner Slider widget/////////////////////////////////
    class Powr_Banner_Slider extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_banner_slider',
                             __( 'POWr Banner Slider' ),
                             array( 'description' => __( 'Banner Slider by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-banner-slider' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Banner Slider.
        </p>
        <p>
    ...
    function register_powr_banner_slider() {
      register_widget( 'Powr_Banner_Slider' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_banner_slider' );
    //Create short codes for adding plugins anywhere:
    function powr_banner_slider_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-banner-slider' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-banner-slider' label='$label'></div>";
      }else{
      	"<div class='powr-banner-slider'></div>";
      }
    }
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_banner_slider_menu' );
    function powr_banner_slider_menu() {
      add_menu_page( 'POWr Banner Slider', 'POWr Banner Slider', 'manage_options', 'powr-banner-slider-settings', 'powr_banner_slider_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_banner_slider_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/banner-slider';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/banner-slider';
      }
    ...
    register_activation_hook(__FILE__, 'powr_banner_slider_plugin_activate');
      add_action('admin_init', 'powr_banner_slider_plugin_redirect');

      function powr_banner_slider_plugin_activate() {
      add_option('powr_banner_slider_plugin_do_activation_redirect', true);
      }
      ...
      function powr_banner_slider_plugin_redirect() {
      if (get_option('powr_banner_slider_plugin_do_activation_redirect', false)) {
          delete_option('powr_banner_slider_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-banner-slider-settings&' );
          }
       }
      }
